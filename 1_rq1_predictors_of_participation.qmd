...
for (i in seq_along(rq1y_twin)){
  ...
  twinmodels[[i]] = glm(formula, data = df_rq1_long, family = binomial)
...
I've removed the intercept from 
...
for (i in seq_along(rq1y_twin)){
  ...
  twinmodels_predictions[[i]]$prediction = twinmodels[[i]] %>% predict(., type = "link") - twinmodels[[i]]$coefficients[1] # removed intercept term
...
ggsave(file.path("plots", "1_rq1_variable_importance_twin-level.pdf"), width = 14, height = 7)
...
heatmap_data = glm_model_comparison_results_df %>%
  ...
  mutate(
    ...
    Delta_AUC_fill = case_when(
      Variables_Dropped %in% c("astress", "adrink", "atwmed1") ~ -999,  # Special value for grey
...
gap_vars = paste0("GAP_", 1:2)  # Create 3 empty segments for gap
all_vars = c(levels(heatmap_data$Variables_wrapped), gap_vars)
...
ggplot(heatmap_data, aes(x = Variables_wrapped, y = outcome_labeled)) +
  geom_tile(aes(fill = Delta_AUC_fill, 
                color = ifelse(Variables_Dropped %in% c("astress", "adrink", "atwmed1"), "lightgrey", "black")), 
            size = .5) +
  scale_color_identity() +
  geom_text(aes(label = ifelse(Wald_p < 0.05, gsub("^0\\.", ".", sprintf("%.3f", Delta_AUC)), "")), 
            size = 1.5, 
            color = "white") +
  # Add custom y-axis labels in the gap
  coord_polar(theta = "x", start = -.05, direction = 1) +
  geom_text(data = y_labels_data, 
...
ggsave(file.path("plots", "1_rq1_heatmap_twinparticipationpredictors.pdf"), width = 8, height = 8)
