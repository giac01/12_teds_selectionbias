...
ggplot(heatmap_data, aes(x = Variables_wrapped, y = outcome_labeled)) +
  geom_tile(aes(fill = Delta_AUC_fill, 
                color = ifelse(Variables_Dropped %in% c("astress", "adrink", "atwmed1", "alookels"), "azure2", "black")), 
            size = .5) +
...
model_metrics = do.call("rbind.data.frame", glm_model_comparison_results) %>%
  ...
  mutate(
    outcome = rq1y_twin_labels_clean[match(.$outcome, rq1y_twin1)],
    outcome = factor(outcome, levels = rq1y_twin_labels_clean)
  ) 
...
ggplot(plot_data, aes(y = outcome, x = value_scaled, fill = metric_label)) +
  ...
  theme(
    legend.position = "bottom",
    axis.text.y = element_text(size = 10, angle = 45, margin = margin(r = 10, unit = "pt")),
...
save_plot("1_rq1_variable_importance", width = 14, height = 7)
...
save_plot("1_prediction_accuracy", width = 9, height = 6)
