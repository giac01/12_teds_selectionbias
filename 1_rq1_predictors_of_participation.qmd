...
Analyses run in docker container bignardig/tidyverse451:v6
...
```{r}
 ...
 # system.time({
 # test = twinmodels[[3]] %>% glm_model_comparison_robust
 # })


glm_model_comparison_results = parallel::mclapply(twinmodels, glm_model_comparison_robust, mc.cores = ncores_use)
...
ggplot(heatmap_data, aes(x = Variables_wrapped, y = outcome_labeled)) +
  geom_tile(aes(fill = Delta_AUC_fill, 
                color = ifelse(Variables_Dropped %in% c("astress", "atwmed1", "adrink", "adadagetw", "anyngsib","afasoc2"), "azure2", "black")), 
            size = .5) +
