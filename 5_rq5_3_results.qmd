...
fmi_original =  map_dfr(rq5y, function(v) {                                     # starts loop for each 
    # browser()                                                                 # For checking this code...

    res = df_rq5_imputed %>%
      group_by(.imp) %>%
      summarise(
        n = sum(!is.na(!!sym(v))),                                              # Number of imputed values
        Q = mean(!!sym(v), na.rm = TRUE),                                       # Vector of imputed means
        U = stats::var(!!sym(v), na.rm = TRUE) / n                              # Vector of imputed sample mean variances
        ) 

    result = mice::pool.scalar(Q = res$Q, U = res$U)
...
fmi_original %>%
  mutate(variable = rq5y_labels_short[match(variable, rq5y)]) %>%
  gt() %>%
...
```
[📊 View Plot](/plots/pngs/5_correlation_residuals.png)
...
[📊 View Plot](/plots/pngs/5_correlations_unimputed.png)
