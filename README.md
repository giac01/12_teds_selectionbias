...
**[View Combined Supplement](0_supplement.html)** - Single-page version assembling all supplementary sections (generated by [`build_supplement.py`](build_supplement.py))

---

## Building the Combined Supplement

[`build_supplement.py`](build_supplement.py) assembles `0_supplement.html` by extracting specific sections from the already-rendered Quarto HTML files — no R code is re-run. It requires the relevant `.qmd` files to have been rendered first.
