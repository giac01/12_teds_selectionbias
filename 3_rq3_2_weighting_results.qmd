---
title: "RQ3: Original vs Weighted Results"
format: 
  html:
...
save_plot("3_correlation_residuals", width = 8, height = 8, trim = TRUE)
...
cor_table = bootstrap_summary %>%
  ...
  filter(xvar != yvar) %>%
  mutate(
    x_var_label = rq5y_labels_short[match(xvar, rq5y)],
    y_var_label = rq5y_labels_short[match(yvar, rq5y)],
  ...
  tab_footnote(
    footnote = md("Correlation difference = Cor<sub>weighted</sub> - Cor<sub>original</sub>"),
...
save_plot("3_correlations_unweighted", width = 8, height = 8, trim = TRUE)
...
check_models_males = lapply(weighted_comparisons, function(x) x$ace[[1]]) %>%
                    bind_rows(., .id = "bootid")
check_models_females = lapply(weighted_comparisons, function(x) x$ace[[3]]) %>%
                    bind_rows(., .id = "bootid")
check_models = rbind.data.frame(check_models_males, check_models_females)

# check_models$par %>% table()
# check_models %>%
#   filter(par == "status_code") %>%
#   pull(value) %>% table()

check_models %>%
  data.frame() %>%
  filter(par == "status_code" & value != 0) %>%
  group_by(name, group) %>%
  count(value) %>%
  ungroup() %>%
  mutate(
  ...
  rename("status_code outcome" = "value") %>%
  knitr::kable()
...
ace_results_summary %>%
  ...
  select(-ymin_weighted, -ymax_weighted, -ymin_unweighted, -ymax_unweighted, -pval_weighted, -pval_unweighted, -pval_adj_weighted, -pval_adj_unweighted) %>%
  select(name_clean, par, y_weighted, y_unweighted, y_difference, ymin_difference, ymax_difference, pval_difference, pval_adj_difference, sex) %>%
  ...
  tab_footnote(
    footnote = "P values are Bonferroni-Holm adjusted across all tests.",
...
ace_results_summary %>%
  ...
  labs(
    title = "Weighted (W) or Original (Unweighted; O) ACE Estimates",
    y     = "Proportion",
    x     = NULL,
    fill  = "Component",
    tag   = "A"
...
save_plot("3_ace_comparison", width = 8, height = 5, trim = TRUE)
