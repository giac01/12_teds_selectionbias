---
title: "S7 RQ3: original vs (attritioned + IP weighted)"
format:
  html:
...
heatmap_data_smd = weighted_comparisons_df %>%
 ...
 ggplot(heatmap_data_smd, aes(x = Variables_wrapped, y = outcome_labeled)) +
  ...
  labs(
    subtitle = expression(frac(bar(X)[attritioned + weighted] - bar(X)[original], SD[original])),
    tag = "A2"
  ) +
  theme_minimal() +
  theme(
...
save_plot("7_rq7_circular_heatmap_smd", width = 8, height = 8, trim = TRUE)

```

[📊 View Full Resolution SMD Heatmap (PNG)](plots/pngs/7_rq7_circular_heatmap_smd.png)
...
ggplot(heatmap_data_var, aes(x = Variables_wrapped, y = outcome_labeled)) +
  ...
  labs(
    subtitle = expression(frac(Var[attritioned + weighted] - Var[original], Var[original]) ~ "\u00D7" ~ 100),
    tag = "B2"
  ) +
  theme_minimal() +
  theme(
...
save_plot("7_rq7_circular_heatmap_var", width = 8, height = 8, trim = TRUE)

```

[📊 View Full Resolution Variance Heatmap (PNG)](plots/pngs/7_rq7_circular_heatmap_var.png)
...
patchwork::wrap_plots(
  ...
  ncol = 4) +
  plot_annotation(
    title = expression(Cor[attritioned + weighted] ~ - ~ ~ Cor[original]),
    theme = theme(
...
save_plot(
  "7_rq7_correlations", width = 9, height = NA, trim = TRUE, trim_margin = 0)



```

[📊 View Full Resolution Correlation Plots (PNG)](plots/pngs/7_rq7_correlations.png)
...
cor_table = bind_rows(correlation_comparisons, .id = "timepoint") %>%
  ...
  tab_footnote(
    footnote = md("Correlation difference = Cor<sub>attritioned + weighted</sub> - Cor<sub>original</sub>"),
...
ace_estimates %>%
  ...
  mutate(
    ...
    group = ifelse(group == "weighted", "Weighted", "Original")
  ) %>%
  pivot_wider(
  ...
  select(dataset_label, name_clean, par, sex,
         ...
         y_Weighted, ymin_Weighted, ymax_Weighted) %>%
  ...
  tab_spanner(
    label = "Attritioned + Weighted",
    columns = c(y_Weighted, ymin_Weighted, ymax_Weighted)
  ) %>%
  tab_options(
...
ace_differences_tbl %>%
  ...
  tab_footnote(
    footnote = "Difference = (Attrition + Weighted) − Original. Adjusted P values are Bonferroni-Holm corrected within each sex, dataset, and parameter (A, C or E) group",
    locations = cells_column_labels(columns = pval_adj)
  ) %>%
  tab_options(
    table.font.size = px(9)
...
ace_estimates %>%
  ...
  labs(
    title = "Attritioned + Weighted vs Original",
    x = NULL,
    y = "Proportion",
    fill = "Component",
    tag = "B"
...
save_plot("7_rq7_ace_comparison", width = 5, height = 8)

```

[📊 View Full Resolution ACE Comparison Plot (PNG)](plots/pngs/7_rq7_ace_comparison.png)
