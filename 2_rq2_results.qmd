...
for (i in seq_along(rq1y_twin1)){
  ...
  filter = as.numeric(df[[rq1y_twin1[i]]])==0 # 0 = not present 
  
  attritioned_datasets[[i]] = df %>% 
    select(all_of(rq2y))
  
  attritioned_datasets[[i]][which(filter),] = NA
...
## Toddlerhood Outcomes: Means, SMDs, and Variances

For each toddlerhood outcome, the table below compares the "original" (full) sample to the "attritioned" sample created by setting that outcome to missing for participants who did not take part in a given wave. Row groups correspond to each attritioned wave. Reported for each outcome are the raw mean difference, the standardized mean difference (SMD), and the percentage change in variance (attritioned relative to original), each with bootstrap 95% CIs and Bonferroni-Holm adjusted p-values; statistically significant increases are shaded red and decreases blue.
...
variable_comparisons_df %>%
  ...
  # Highlight significant results - positive effects (light red)
  tab_style(
    style = cell_fill(color = "#ffd9d1"),  # Light red for positive effects
    locations = cells_body(
      columns = c(y_md, ymin_md, ymax_md, pval_adj_md),
  ...
  # Highlight significant results - negative effects (light blue)
  tab_style(
    style = cell_fill(color = "#b6c3e0"),  # Light blue for negative effects
    locations = cells_body(
      columns = c(y_md, ymin_md, ymax_md, pval_adj_md),
      rows = pval_adj_md < 0.05 & y_md < 0
    )
  ...
  # Add footnotes
  tab_footnote(
    footnote = md("<em>Note.</em> Est = Estimate, LB = Lower Bound 95% Confidence Interval, UB = Upper Bound 95% Confidence Interval. Significant (p<sub>Bonferroni-Holm</sub>) effects are highlighted in red (increases) or blue (decreases)."),
    # locations = cells_column_spanners(),
    placement = "right"
  ) %>%
  tab_footnote(
...
```{r}
  # Create circular heatmap for SMD
   heatmap_data_smd = variable_comparisons_df %>%
    filter(stat == "smd") %>%
    mutate(
      variable = ifelse(variable == "Peer problems", "Peer prob.", variable),
      ...
      Variables_wrapped = factor(Variables_wrapped, levels = c("Maternal\nEducation", "Conduct\nproblems", "Peer\nprob.", "Parent-report\ncognition", "Parent-admin\ncognition", "Vocabulary", "Hyperactivity", "Grammar",
  "Emotional\nproblems", "Prosocial\nbehavior")),
  ...
  ggplot(heatmap_data_smd, aes(x = Variables_wrapped, y = outcome_labeled)) +
    ...
    geom_text(
      aes(label = ifelse(pval_adj < 0.05,
          ...
          size = ifelse(outcome_labeled == "Y4", 2, 4.25)
      ),
      color = "black"
    ) +
    scale_size_identity() +
    ...
    theme(
      ...
      plot.subtitle = element_text(hjust = 0.5, size = 17, margin = margin(b = -20)),
    ...
    ) +
    {
      ...
      lapply(seq_along(labels), function(i) {
        annotate("text", x = 0.5, y = y_positions[i],
                 ...
                 size = 3.7,
                 hjust = 1, angle = 7, fontface = "bold")
      })
    }

  save_plot("2_rq2_circular_heatmap_smd2", width = 8, height = 8, trim = TRUE)

```

[📊 View Full Resolution SMD Heatmap (PNG)](plots/pngs/2_rq2_circular_heatmap_smd2.png)
...
```{r}
  ...
  ggplot(heatmap_data_var, aes(x = Variables_wrapped, y = outcome_labeled)) +
    ...
    geom_text(
      aes(label = ifelse(pval_adj < 0.05,
                         paste0(sprintf("%.1f", y * 100), ""), ""), # ADD THE PERCENTAGE SYMBOL TO THE FIRST EMPTY "" IF NEEDED AAIN
          size = ifelse(outcome_labeled == "Y4", 2, 4.2)
      ),
      color = "black"
    ) +
    scale_size_identity() +
  ...
  save_plot("2_rq2_circular_heatmap_var2", width = 8, height = 8, trim = TRUE)

```

[📊 View Full Resolution Variance Heatmap (PNG)](plots/pngs/2_rq2_circular_heatmap_var2.png)
...
## Toddlerhood Outcomes: Correlations

Below is a results table for the change in correlations. For every pair of toddlerhood outcomes, we compare the correlation in the attritioned sample to the correlation in the original sample (row groups = attrition timepoint), reporting the difference with bootstrap 95% CIs and unadjusted/Bonferroni-Holm adjusted p-values; significant increases are shaded green and decreases red.
...
save_plot(
  "2_rq2_2_correlations_original_simple", width = 6.5, height = 6.5)

```

[📊 View Plot](plots/pngs/2_rq2_2_correlations_original_simple.png)
...
## Toddlerhood Outcomes: ACE Estimates and Differences

The table below compares the ACE (Additive genetic, shared environment, non-shared environment) variance component estimates from twin models fitted to the original vs. attritioned toddlerhood samples, separately by sex and attrition timepoint. For each outcome and component (A, C, E) it shows the original and attritioned estimates, their difference with bootstrap 95% CI, and a Bonferroni-Holm adjusted p-value (adjusted within each sex/parameter/timepoint group); significant differences are highlighted green.
...
```{r}
# For each ACE component × variable, how many of the sex × timepoint
# combinations show a positive vs negative change, and how many are significant?
bootstrap_summary_df_ace %>%
  filter(group == "Difference", par %in% c("a", "c", "e")) %>%
  mutate(
    par        = toupper(par),
    name_clean = rq2y_labels_short[match(name, rq2y)]
  ) %>%
  ...
  tab_footnote(
    footnote = md("<em>Note.</em> N = number of sex × timepoint combinations. N &uarr; / N &darr; = number with positive/negative change (Attritioned − Original). N sig = number with p<sub>unadj</sub> < 0.05. Cells highlighted when all combinations agree in direction."),
    placement = "right"
  )
```
...
bootstrap_summary_df_ace %>%
  ...
  mutate(
    ...
    sex           = str_to_title(sex),
    name_clean    = rq2y_labels_short[match(name, rq2y)],
    name_clean    = factor(name_clean, levels = rev(rq2y_labels_short)),
    dataset_label = factor(dataset_label, levels = rq1y_twin_labels_clean_extrashort),
  ...
  geom_pointrange(position = position_dodge(width = 0.7), size = 0.3, linewidth = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40", linewidth = 0.5) +
  facet_grid(sex ~ par, scales = "free_x") +
  scale_color_manual(
    values = c(RColorBrewer::brewer.pal(8, "Dark2"), "#666666"),
    name = "Attrition timepoint"
  ) +
  scale_alpha_manual(values = c("TRUE" = 1, "FALSE" = 0.3), guide = "none") +
  labs(
    title    = "Changes in ACE Estimates by Variable",
    subtitle = "Attritioned \u2212 Original (95% CI). Dashed line = no change. Faded = p\u209a > 0.05.",
    x        = "ACE Difference",
    y        = NULL
  ) +
  theme_bw() +
...
save_plot("2_rq2_ace_forest", width = 8, height = 7)
```

[View full-size plot](plots/pngs/2_rq2_ace_forest.png){target="_blank"}
...
response_statistics = df %>%
  select(randomtwinid, all_of(rq2y), all_of(rq1y_twin1)) %>% 
  mutate(all1 = 1) %>% 
  pivot_longer(cols = all_of(rq2y), names_to = "variable", values_to = "score") %>% 
...
response_statistics %>%
  mutate(
    variable = rq2y_labels_short[match(variable, rq2y)],
    variable = factor(variable, levels = rq2y_labels_short),
  ...
  labs(
    title = "Toddlerhood Measures"
  )

save_plot("2_attrition_numbers", width = 7.3, height = 4.3)

```

[📊 View Full Resolution Attrition Numbers Plot (PNG)](plots/pngs/2_attrition_numbers.png)
...
response_statistics %>%
  mutate(
    ...
    timepoint = rq1y_twin_labels_clean_extrashort[match(timepoint, rq1y_twin1)],
    timepoint = ifelse(is.na(timepoint), "Original", timepoint),
    # Combine n and percent into formatted string
    n_pct = paste0(format(n, big.mark = ","), " (", sprintf("%.1f%%", percent * 100), ")")
  ) %>%
  select(variable, timepoint, n_pct) %>%
  pivot_wider(names_from = timepoint, values_from = n_pct) %>%
  arrange(variable) %>%
  gt() %>%
  cols_label(
    variable = "Variable"
  ...
  tab_spanner(
    label = "Sample Size (% of Original) by Attrition Timepoint",
    columns = -variable
  ) %>%
  tab_style(
    style = cell_text(size = px(10)),
  ...
  tab_footnote(
    footnote = "Original = full sample before any attrition. Other columns show sample sizes (and percentage of original) after removing participants who did not take part at that timepoint.",
    placement = "right"
  ) %>%
  cols_move_to_start(columns = c(variable, Original))
...
model_metrics_ordered = model_metrics %>%
  mutate(
    ...
    year_num = if_else(is.na(year_num), Inf, year_num),
...
[📊 View Full Resolution Prediction Accuracy Plot (PNG)](plots/pngs/2_prediction_accuracy.png)
...
[📊 View Full Resolution Variable Importance Plot (PNG)](plots/pngs/2_rq2_variable_importance.png)
...
## Effect of attritioning maternal education removing HNC and HND
...
summary_stats = df %>% 
  mutate(mat_edu = amohqualn1) %>% 
  select(all_of(rq1y_twin1),mat_edu) %>% 
  mutate(fullsample = 1) %>% 
  pivot_longer(cols = !mat_edu) %>% 
  group_by(name, value) %>% 
  summarise(
    mean = mean(mat_edu, na.rm = TRUE),
    var  = sd(mat_edu, na.rm = TRUE)^2,
    entropy = entropy(mat_edu),
    n    = dplyr::n()
  ) %>% 
  filter(value == 1)
...
summary_stats %>% 
  mutate(
    name = rq1y_twin_labels_clean_extrashort[match(name, rq1y_twin1)],
    name = case_when(
      is.na(name) ~ "Full Sample",
      .default = name
    )
  ) %>% 
  arrange(var) %>% 
...
### Combining HNC and HND with A-levels

HNC and HND qualifications are vocational and sit ambiguously between A-levels and undergraduate degrees in the original ordinal coding. Here they are collapsed into the A-level category to create a cleaner ordinal scale. The table shows the mean, variance, and entropy of the recoded maternal education variable for participants at each study timepoint and for the full sample. Higher entropy indicates a more evenly distributed education profile among participants; a decline in entropy relative to the full sample would indicate that attrition is concentrating the sample toward certain education levels.

```{r}
# table(df$amohqualn1,  df$amohqual)

df3 = df %>% 
  mutate(
    mat_edu = case_when(
      .default = amohqualn1,
      amohqualn1 == 5 ~ 4,  # HNC -> A level
      amohqualn1 == 6 ~ 4,  # HND -> A level
      amohqualn1 == 7 ~ 5   #undergraduate degree
...
table(df$amohqualn1, df3$mat_edu)

```

### Dichotomising education into degree vs. no degree

Here maternal education is reduced to a binary variable: whether or not the mother holds an undergraduate or postgraduate degree. This is a common simplification in the literature and avoids assumptions about the spacing of the ordinal scale. The table shows the proportion of mothers with a degree (mean), variance, and entropy among participants at each timepoint and in the full sample. A rising proportion of degree-educated mothers across timepoints would indicate that attrition disproportionately removes families from lower education backgrounds.

```{r}

df4 = df %>%
  mutate(
    mat_edu_degree = case_when(
      amohqualn1 >= 7 ~ 1L,  # undergraduate degree or above
      amohqualn1 <  7 ~ 0L,
      .default = NA_integer_
    )
  ) %>%
  select(all_of(rq1y_twin1), mat_edu_degree)

summary_stats_degree = df4 %>%
  mutate(fullsample = 1) %>%
  pivot_longer(cols = !mat_edu_degree) %>%
  group_by(name, value) %>%
  summarise(
    mean    = mean(mat_edu_degree, na.rm = TRUE),
    var     = sd(mat_edu_degree, na.rm = TRUE)^2,
    entropy = entropy(mat_edu_degree),
    n       = dplyr::n()
  ) %>%
  filter(value == 1)
...
table(df$amohqualn1, df4$mat_edu_degree)

```

## Distribution of maternal education: full sample vs. Y26 Webtest participants

The plot below compares the distribution of maternal education (`amohqual`) between the full TEDS sample and those who participated at the Y26 Webtest (`zcdata1 == 1`). Bars show the proportion of mothers at each education level within each group. 

```{r}

df %>%
  # filter(twin==1) %>% 
  select(zcdata1, amohqual) %>%
  mutate(fullsample = 1) %>%
  pivot_longer(cols = c(fullsample, zcdata1), names_to = "group", values_to = "participated") %>%
  filter(participated == 1, !is.na(amohqual)) %>%
  mutate(group = if_else(group == "fullsample", "Full sample", "Y26 Webtest participants")) %>%
  group_by(group, amohqual) %>%
  summarise(n = dplyr::n(), .groups = "drop") %>%
  group_by(group) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
  mutate(amohqual = factor(amohqual, levels = levels(df$amohqual))) %>%
  ggplot(aes(x = amohqual, y = prop, fill = group)) +
  ...
  scale_fill_manual(values = c("Full sample" = "#d53031", "Y26 Webtest participants" = "#00929c")) +
  theme_bw() +
  theme(
