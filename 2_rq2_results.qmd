...
```{r}
  ...
  ggplot(heatmap_data_smd, aes(x = Variables_wrapped, y = outcome_labeled)) +
    ...
    labs(
      subtitle = expression(frac(bar(X)[attritioning] - bar(X)[original], SD[original])),
      tag = "A0"
    ) +
    theme_minimal() +
    theme(
...
```{r}
  ...
  ggplot(heatmap_data_var, aes(x = Variables_wrapped, y = outcome_labeled)) +
    ...
    labs(
      subtitle = expression(frac(Var[attritioning] - Var[original], Var[original]) ~ "\u00D7" ~ 100),
      tag = "B0"
    ) +
    theme_minimal() +
    theme(
...
response_statistics = df %>%
  select(randomtwinid, rq2y, rq1y_twin1) %>% 
  mutate(all1 = 1) %>% 
