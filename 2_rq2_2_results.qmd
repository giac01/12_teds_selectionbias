...
variable_comparisons    = readRDS(file = file.path("results", "variable_comparisons.Rds"))

variable_comparisons_df = readRDS(file = file.path("results", "variable_comparisons_df.Rds"))
...
ace_comparisons = readRDS(file = file.path("results", "ace_comparisons.Rds"))
...
ace_boot_results_df = lapply(ace_comparisons, function(x) x$boot_results_df)

ace_boot_results_df = map_dfr(ace_boot_results_df, ~ map_dfr(.x, ~ map_dfr(.x, ~ .x, .id = "component"), .id = "dataset"), .id = "variable")
...
attritioned_ace_estimates = ace_boot_results_df %>%
  pivot_longer(cols = c(a,c,e), names_to = "parameter") %>%
  group_by(variable, dataset, parameter) %>%
  summarise(
    estimate = mean(value),
