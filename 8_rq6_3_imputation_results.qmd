...
save_plot(
  "8_rq6_correlations", width = 9, height = 2.7, trim = TRUE, trim_margin = 0)
...
# Flatten RGBA to white background (prevents black padding when stacking)
img1 = image_flatten(img1, operator = "Over")
img2 = image_flatten(img2, operator = "Over")
img3 = image_flatten(img3, operator = "Over")

# Pad narrower images with white on the right to match the widest
max_width = max(image_info(img1)$width, image_info(img2)$width, image_info(img3)$width)
img1 = image_extent(img1, paste0(max_width, "x", image_info(img1)$height), gravity = "West", color = "white")
img2 = image_extent(img2, paste0(max_width, "x", image_info(img2)$height), gravity = "West", color = "white")
