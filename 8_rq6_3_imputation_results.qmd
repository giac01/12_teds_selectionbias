---
title: "S8 RQ3: original vs (attritioned + imputed)"
format:
  html:
...
### Table of all variables used in multiple imputation (RQ3)

Note that imputation was performed separately across zygosity-sex groups (i.e., MZ males, MZ females, DZ males, ...) therefore sex and zygosity are omitted from imputation models. 

Because imputations were performed separetely for each attriitoning time point, we calculate FMI for each timepoint and average across.

The table below lists every variable included in the multiple imputation models for RQ3, with its description, TEDS code, observed range/levels, percentage missing in the original sample, mean fraction of missing information (FMI) across timepoints, and study wave.

```{r imputation-vars-table}
...
# % missing cases in original df
pct_missing = sapply(rq6_imputation_vars, function(var) {
  if (var %in% colnames(df)) {
    round(100 * mean(is.na(df[[var]])), 1)
  } else {
    NA_real_
  }
})

# Fraction of Missing Information (FMI) for sample means across attrition timepoints (numeric vars only)
fmi_by_var = sapply(rq6_imputation_vars, function(v) {
  # Only compute for numeric variables; factors have no meaningful mean FMI
  if (!v %in% colnames(df) || !is.numeric(df[[v]])) return(NA_real_)
  # Calculate FMI for each attritioning timepoint (imputations were run separately for each attritioning timepoint)
  fmi_vals = sapply(seq_along(imputed_mice), function(tp) {
    # browser()
    imp_list = imputed_mice[[tp]]
    if (!v %in% colnames(imp_list[[1]])) return(NA_real_)
    Q = sapply(imp_list, function(d) mean(d[[v]], na.rm = TRUE))
    U = sapply(imp_list, function(d) {
      n_obs = sum(!is.na(d[[v]]))
      if (n_obs < 2) return(NA_real_)
      stats::var(d[[v]], na.rm = TRUE) / n_obs
    })
    tryCatch(
      mice::pool.scalar(Q = Q, U = U)$fmi,
      error = function(e) NA_real_
    )
  })
  mean(fmi_vals, na.rm = TRUE)
})
...
gt(v_impute) %>%
  ...
  cols_hide(N) %>%
  fmt_percent(columns = `FMI (mean)`, decimals = 0) %>%
  fmt_number(columns = `Missing (%)`, decimals = 0, pattern = "{x}%") %>%
  tab_source_note(
    source_note = "Note: Range or Level shows min-max values for numeric variables or factor levels for categorical variables (reference level marked with *). Variable codes with an asterisk (*) have been derived or modified from the original dataset. Missing (%) is the percentage of missing cases in the original sample. FMI (mean) is the fraction of missing information for the sample mean, averaged across attrition timepoints; NA for factor variables."
  ) %>%
  tab_options(
    table.width = "70%"
  ) %>%
...
heatmap_data_smd = imputed_comparisons_df %>%
 ...
 ggplot(heatmap_data_smd, aes(x = Variables_wrapped, y = outcome_labeled)) +
  ...
  labs(
    subtitle = expression(frac(bar(X)[attritioned + imputed] - bar(X)[original], SD[original])),
    tag = "A3"
  ) +
  theme_minimal() +
  theme(
...
ggplot(heatmap_data_var, aes(x = Variables_wrapped, y = outcome_labeled)) +
  ...
  labs(
    subtitle = expression(frac(Var[attritioned + imputed] - Var[original], Var[original]) ~ "\u00D7" ~ 100),
    tag = "B3"
  ) +
  theme_minimal() +
  theme(
...
rq1y_twin_labels_clean_extrashort2[3] = "Y26 (web)"
...
plots = lapply(1:length(correlation_comparisons), function(i) {
  plot_lower_triangular_matrix2(
    ...
    show_values = TRUE,
    text_size = 4.2,
    title = rq1y_twin_labels_clean_extrashort2[i],
    caption = NULL,
    method = "none"
  ) +
  labs(tag = paste0("C", i)) +
...
save_plot(
  "8_rq6_correlations", width = 9, height = NA, trim = TRUE, trim_margin = 0)

```

[📊 View Full Resolution Correlation Plots (PNG)](plots/pngs/8_rq6_correlations.png)

### GT Results table

```{r gt-correlations-table}
...
[🖼️ Image #1](plots/pngs/8_rq6_correlations_combined.png)
...
```{r attrition-only-plot}

response_statistics %>%
  filter(condition == "Attritioned") %>%
  mutate(
    variable = rq6y_labels[match(variable, rq6y_prefix)],
    variable = factor(variable, levels = rq6y_labels),
    timepoint = factor(timepoint, levels = c("Original", rq1y_twin_labels_clean_extrashort)),
...
save_plot("8_attrition_numbers", width = 6, height = 2)

```

[📊 View Full Resolution Attrition Plot (PNG)](plots/pngs/8_attrition_numbers.png)
...
## Correlation Matrix of Imputation Model Variables

Correlation matrix (Pearson, pairwise complete observations) of all numeric variables included in the RQ3 multiple imputation models (see [Table of all variables used in multiple imputation (RQ3)]). Categorical/factor variables are not correlation-compatible and are excluded from the matrix below.

```{r imputation-vars-correlation-matrix}
#| fig-width: 26
#| fig-height: 26

numeric_impute_vars = rq6_imputation_vars[sapply(rq6_imputation_vars, function(v) is.numeric(df[[v]]))]

nonnumeric_impute_vars = rq6_imputation_vars[sapply(rq6_imputation_vars, function(v) !is.numeric(df[[v]]))]

df %>% 
  select(all_of(nonnumeric_impute_vars)) %>% 
  apply(.,2,table)

cor_dat = df %>%
  select(all_of(numeric_impute_vars),"asmoke","atwclub") %>% 
  mutate(
    asmoke = as.numeric(asmoke),
    atwclub = as.numeric(atwclub)
  ) %>% 
  select(-ends_with("2")) %>% 
  `colnames<-`(var_to_label(colnames(.)))
...
save_plot("8_rq6_imputation_vars_correlation_matrix", width = 18, height = 18, trim = FALSE)

```

[📊 View Full Resolution Correlation Matrix (PNG)](plots/pngs/8_rq6_imputation_vars_correlation_matrix.png)
