...
save_plot(
  "8_rq6_correlations", width = 12, height = 8)
...
ace_estimates %>%
  mutate(
    ...
    group = case_when(
      group == "df1" ~ "Original",
      group == "df2" ~ "Attrition + Imputed",
      TRUE ~ group
    ),
    group = factor(group, levels = c("Attrition + Imputed", "Original")),
...
save_plot("8_rq6_ace_comparison", width = 10, height = 7)
