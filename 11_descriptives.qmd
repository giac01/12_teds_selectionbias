...
v_rq1y$Description = str_remove(v_rq1y$Description,"\\(1st.*")
...
v_rq5_indexed = cbind(row_group = "RQ5 Variables", row_id = (nrow(v_rq1x)+nrow(v_rq1y)+nrow(v_rq2)+1):(nrow(v_rq1x)+nrow(v_rq1y)+nrow(v_rq2)+nrow(v_rq5)), v_rq5)

v_rq_combined = rbind(v_rq1x_indexed, v_rq1y_indexed, v_rq2_indexed, v_rq5_indexed)

gt(v_rq_combined) %>%
  tab_row_group(
    label = "RQ4-5 Variables",
  ...
  tab_row_group(
    label = "RQ1-Y (participation indicator variables)",
    rows = row_group == "RQ1 Outcome Variables"
  ) %>%
  tab_row_group(
    label = "RQ1-X (baseline variables used to predict later participation)",
